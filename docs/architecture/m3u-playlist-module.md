...
M3U imports preserve XMLTV URLs declared on the `#EXTM3U` header. The parser
recognizes `url-tvg`, `x-tvg-url`, and `tvg-url` attributes, including quoted
and unquoted values, and stores the normalized list on `Playlist.epgUrls`.

When a playlist is added or refreshed, `PlaylistEffects` asks
`PlaylistsService.mergePlaylistEpgUrlsIntoSettings()` to merge those URLs into
`settings.epgUrl`, mirror the updated settings into Electron app state, and
trigger a desktop EPG fetch for newly discovered playlist-declared sources.
...
- Favorites toggle
- EPG sidebar
- Source-declared alternate stream lines for the active channel
...
### Alternate Stream Lines

Some M3U providers represent line changes as repeated adjacent `#EXTINF`
entries for the same logical channel, while others put multiple stream URLs in
one raw item block. Import normalization keeps the first URL as the canonical
channel URL and stores all discovered choices in `Channel.alternateStreams`.
Non-adjacent entries without `tvg-id` can also be merged when they share the
same `tvg-name` and their group names belong to the same family, for example
`卫视` and `卫视-MCP`. This covers providers that publish backup provider lines
as a later provider-specific group while still keeping same-name channels in
unrelated groups separate.

The player renders a line switcher only when more than one stream is available.
Selecting a line dispatches `ChannelActions.setActiveChannelStream`, updates the
active channel URL and per-line HTTP headers, clears archived playback override
state, and reopens the selected URL for MPV/VLC when an external player is
configured. Channel highlighting, favorites, recent history, remote navigation,
and global collections treat all alternate URLs as identities of the same
logical channel.
