...
export class VideoPlayerComponent implements OnInit, OnDestroy {
    ...
    readonly activeChannelStreams = computed(() =>
        this.getChannelStreams(this.activeChannel())
    );
    ...
    constructor() {
        ...
        effect(() => {
            ...
            const channelIdentity = getChannelPrimaryUrl(activeChannel);
            const nextKey = `${playlistId}::${channelIdentity}`;
        ...
        effect(() => {
            ...
            if (isChannelUrlIdentity(activeChannel, targetUrl)) {
                this.clearConsumedRecentChannelState();
                return;
            }

            const matchedChannel = channels.find((channel) =>
                isChannelUrlIdentity(channel, targetUrl)
            );
            if (!matchedChannel) {
                return;
            }
    ...
    ngOnInit(): void {
        ...
        this.statusSubscription = combineLatest([
            ...
            const currentIndex = channels.findIndex((channel) =>
                isChannelUrlIdentity(
                    channel,
                    getChannelPrimaryUrl(activeChannel)
                )
            );
            ...
            .subscribe({
                next: ({ channels, activeChannel }) => {
                    const nextChannel = getAdjacentChannelItem(
                        ...
                        getChannelPrimaryUrl(activeChannel),
                        direction,
                        (channel) => getChannelPrimaryUrl(channel)
                    );
    ...
    switchActiveChannelStream(url: string): void {
        const activeChannel = this.activeChannel();
        if (!activeChannel || !url || url === activeChannel.url) {
            return;
        }

        this.store.dispatch(ChannelActions.setActiveChannelStream({ url }));

        if (
            activeChannel.radio !== 'true' &&
            this.isExternalPlayer(this.playerSettings.player)
        ) {
            const selectedStream = activeChannel.alternateStreams?.find(
                (stream) => stream.url === url
            );
            const channelForSelectedStream = {
                ...activeChannel,
                url,
                http: {
                    ...activeChannel.http,
    ...
    private async persistRecentlyViewedChannel(
        ...
        const recentlyViewedItem: M3uRecentlyViewedItem = {
            source: 'm3u',
            id: getChannelPrimaryUrl(channel),
            url: getChannelPrimaryUrl(channel),
    ...
    private getChannelStreams(channel: Channel | null | undefined) {
        if (!channel?.url) {
            return [];
        }

        const streams = (channel.alternateStreams ?? [])
            .map((stream, index) => ({
                label: stream.label?.trim() || `Line ${index + 1}`,
                url: stream.url?.trim(),
            }))
        ...
        if (!streams.some((stream) => stream.url === channel.url)) {
            return [
                {
                    label: 'Line 1',
                    url: channel.url,
