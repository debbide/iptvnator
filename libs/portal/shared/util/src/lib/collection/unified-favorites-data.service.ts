...
export class UnifiedFavoritesDataService {
    ...
    async removeFavorite(item: UnifiedCollectionItem): Promise<void> {
        switch (item.sourceType) {
            case 'm3u': {
                ...
                const targetIds = this.getM3uItemFavoriteIds(item);
                const filtered = (
                    (playlist.favorites as string[]) ?? []
                ).filter((favoriteId) => !targetIds.has(favoriteId));
    ...
    private async addM3uFavorite(item: UnifiedCollectionItem): Promise<void> {
        const favoriteId = this.getM3uItemPrimaryFavoriteId(item);
        if (!favoriteId) {
            return;
        }
        ...
        const targetIds = this.getM3uItemFavoriteIds(item);
        if (currentFavorites.some((favorite) => targetIds.has(favorite))) {
            return;
        }
    ...
    async reorder(
        ...
        if (
            ...
            await this.setM3uFavorites(
                options.playlistId,
                items
                    .map((item) => this.getM3uItemPrimaryFavoriteId(item) ?? '')
            ...
            Array.from(groupedItems.entries()).map(
                async ([playlistId, playlistItems]) => {
                    ...
                    playlistItems.forEach((item) => {
                        ...
                        this.getM3uItemFavoriteIds(item).forEach((value) =>
                            targetIds.add(value)
                        );
                    });
                    const currentFavorites = Array.isArray(playlist?.favorites)
    ...
    private async extractM3uFavorites(
        ...
        channels.forEach((channel) => {
            getChannelUrlIdentities(channel).forEach((url) =>
                channelsByFavoriteId.set(url, channel)
            );
            if (channel.id?.trim()) {
        ...
        return favoriteIds
            .map((favoriteId, index) => {
                ...
                const primaryUrl = getChannelPrimaryUrl(channel);
                if (!primaryUrl || seenUrls.has(primaryUrl)) {
                    return null;
                }
                seenUrls.add(primaryUrl);
                const sourceItemId = primaryUrl || channel.id?.trim();
                if (!sourceItemId) {
                    return null;
                }
    ...
    private getM3uItemPrimaryFavoriteId(
        item: UnifiedCollectionItem
    ): string | null {
        return (
            getChannelPrimaryUrl(item.m3uChannel) ||
            item.streamUrl ||
            item.channelId ||
            null
        );
    ...
    private getM3uItemFavoriteIds(item: UnifiedCollectionItem): Set<string> {
        const targetIds = new Set<string>();
        [item.streamUrl, item.channelId, this.getM3uItemPrimaryFavoriteId(item)]
            .map((value) => value?.trim())
            .filter((value): value is string => Boolean(value))
            .forEach((value) => targetIds.add(value));
        getChannelUrlIdentities(item.m3uChannel).forEach((value) =>
            targetIds.add(value)
        );
