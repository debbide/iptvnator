...
describe('PlaylistsService', () => {
    ...
    it('merges playlist-declared EPG URLs into stored settings', async () => {
        const electron = {
            dbSetAppState: jest.fn(async () => undefined),
            fetchEpg: jest.fn(async () => undefined),
        };
        const storage = {
            get: jest.fn(() =>
                of({
                    epgUrl: ['https://existing.example.com/guide.xml'],
                })
            ),
            set: jest.fn(() => of(undefined)),
        };
        ...
        testWindow.electron = electron;
        const service = createService();
        Object.assign(service as object, { settingsStore, storage });

        await service.mergePlaylistEpgUrlsIntoSettings({
            _id: 'playlist-epg',
            title: 'Playlist EPG',
            count: 0,
            ...
            autoRefresh: false,
            epgUrls: [
                'https://new.example.com/guide.xml',
                'https://existing.example.com/guide.xml',
            ],
        } as Playlist);
        ...
        expect(electron.dbSetAppState).toHaveBeenCalledWith(
            'settings',
            JSON.stringify({
                epgUrl: [
                    'https://existing.example.com/guide.xml',
        ...
        expect(electron.fetchEpg).toHaveBeenCalledWith([
            'https://new.example.com/guide.xml',
            'https://existing.example.com/guide.xml',
        ]);
    });

    it('deduplicates stored EPG URLs while merging playlist-declared URLs', async () => {
        const storage = {
            get: jest.fn(() =>
                of({
                    epgUrl: [
        ...
        const service = createService();
        Object.assign(service as object, { storage });
        ...
        await service.mergePlaylistEpgUrlsIntoSettings({
            ...
            lastUsage: new Date('2026-04-11T00:00:00.000Z').toISOString(),
            autoRefresh: false,
            epgUrls: ['https://new.example.com/guide.xml'],
        } as Playlist);
