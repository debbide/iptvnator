...
type PlaylistStorageElectronApi = {
    ...
    fetchEpg?: (urls: string[]) => Promise<unknown>;
};
...
export class PlaylistsService {
    ...
    async mergePlaylistEpgUrlsIntoSettings(playlist: Playlist): Promise<void> {
        const epgUrls = this.normalizeEpgUrls(playlist.epgUrls);
        if (epgUrls.length === 0) {
            return;
        }

        const currentSettings = {
            ...this.readSettingsStoreSnapshot(),
            ...(await this.readStoredSettings()),
        };
        const existingUrls = this.normalizeEpgUrls(currentSettings.epgUrl);
        const mergedUrls = Array.from(new Set([...existingUrls, ...epgUrls]));

        if (this.areEpgUrlsEqual(existingUrls, mergedUrls)) {
            return;
        }
        ...
        await firstValueFrom(
            this.storage.set(STORE_KEY.Settings, nextSettings)
        );
        this.updateSettingsStoreEpgUrls(mergedUrls);

        if (this.electronApi?.dbSetAppState) {
            await this.electronApi.dbSetAppState(
                STORE_KEY.Settings,
                JSON.stringify(nextSettings)
            );
        }

        if (this.electronApi?.fetchEpg) {
            await this.electronApi.fetchEpg(epgUrls);
        }
    }

    private async readStoredSettings(): Promise<Partial<Settings>> {
        try {
            ...
            // Fall through to the SQLite app_state mirror used by Electron.
        }

        try {
            const rawSettings = await this.electronApi?.dbGetAppState?.(
                STORE_KEY.Settings
            );
            return rawSettings ? (JSON.parse(rawSettings) as Settings) : {};
        } catch {
            return {};
        }
    }

    private readSettingsStoreSnapshot(): Partial<Settings> {
        try {
    ...
    private normalizeEpgUrls(value: string[] | string | undefined): string[] {
    ...
    private areEpgUrlsEqual(first: string[], second: string[]): boolean {
        return (
            first.length === second.length &&
            first.every((url, index) => url === second[index])
        );
    }

    private updateSettingsStoreEpgUrls(epgUrls: string[]): void {
        try {
            patchState(this.settingsStore as Parameters<typeof patchState>[0], {
                epgUrl: epgUrls,
            });
        } catch {
            // Tests often stub the signal store with only the properties they use.
        }
    }
