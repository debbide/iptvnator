...
function mapChannelsByFirstUrl(channels: Channel[]): Map<string, Channel> {
    ...
    for (const channel of channels) {
        for (const channelUrl of getChannelUrlIdentities(channel)) {
...
function getActiveChannelListUrl(
    channels: Channel[],
    ...
    return (
        channels.find((channel) => isChannelUrlIdentity(channel, activeUrl))
            ?.url ?? activeUrl
    );
}
...
export class ChannelListContainerComponent implements OnInit, OnDestroy {
    ...
    readonly recentChannelItems = computed<RecentViewItem[]>(() => {
        ...
        const channelsByUrl = mapChannelsByFirstUrl(channels);
        ...
        return [...recentItems]
            ...
            .reduce<RecentViewItem[]>((acc, item) => {
                ...
                seenUrls.add(getChannelPrimaryUrl(channel));
                acc.push({
                    channel,
    ...
    readonly activeChannelListUrl = computed(() =>
        getActiveChannelListUrl(
            this.channelListSignal(),
            this.activeChannelUrl()
        )
    );
