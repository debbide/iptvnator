...
export class GroupsViewComponent {
    ...
    isChannelSelected(channel: Channel): boolean {
        return isChannelUrlIdentity(channel, this.activeChannelUrl());
    }

    isFavorite(channel: Channel): boolean {
        const favorites = this.favoriteIds();
        return getChannelUrlIdentities(channel).some((url) =>
            favorites.has(url)
        );
    }
        ...
        for (const [groupKey, channels] of Object.entries(
            ...
            for (const channel of channels) {
                for (const channelUrl of [
                    channel.url,
                    ...(channel.alternateStreams ?? []).map(
                        (stream) => stream.url
                    ),
                ]) {
                    if (channelUrl && !groupKeys.has(channelUrl)) {
