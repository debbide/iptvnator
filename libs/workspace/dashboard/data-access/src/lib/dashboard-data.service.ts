    ...
    private async loadM3uPlaylistFavorites(
        ...
        const computedItems = items.reduce<DashboardFavoriteItem[]>(
            (acc, channel) => {
                ...
                if (!primaryUrl || seenPrimaryUrls.has(primaryUrl)) {
                    return acc;
                }

                const channelIdFavoritePosition =
                    favoritePositions.get(channelId);
                const matchedFavoriteUrl = getChannelUrlIdentities(
                    channel
                ).reduce<{ position: number; url: string } | undefined>(
                    (match, url) => {
                        const position = favoritePositions.get(url);
                        if (
                            position === undefined ||
                ...
                const matchedFavoriteId =
                    ...
                    (matchedFavoriteUrl === undefined ||
                        channelIdFavoritePosition <=
                            matchedFavoriteUrl.position)
                        ? channelId
                        : matchedFavoriteUrl !== undefined
                          ? matchedFavoriteUrl.url
                          : null;
